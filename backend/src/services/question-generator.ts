...
export class QuestionGenerator {
  ...
  private async selectCorrectAnswer(
    ...
    if (excludeCountries.length > 0) {
      const placeholders = excludeCountries.map((_, i) => `$${i + 1}`).join(', ');
      conditions.push(`country_code NOT IN (${placeholders})`);
  ...
  private async generateIncorrectChoices(
    ...
    if (excludeList.length > 0) {
      const placeholders = excludeList.map((_, i) => `$${i + 4}`).join(', ');
      query += ` WHERE country_code NOT IN (${placeholders})`;
    ...
    params.push(count * 2); // Get more than needed for better selection
    ...
    if (result.rows.length < count) {
      ...
      if (excludeList.length > 0) {
        ...
        additionalQuery += ` WHERE country_code NOT IN (${placeholders})`;
        additionalParams.push(...excludeList);
      }
      
      additionalQuery += ` ORDER BY RANDOM() LIMIT $${additionalParams.length + 1}`;
      additionalParams.push(count);
      
      const additionalResult = await client.query(additionalQuery, additionalParams);
